...
namespace ExtradosStore.Services.Implementations
{
    public class UserService : IUserService
    {
        ...
        public async Task<int> DisableUserService(int idUser)
        {
            ...
            if (!user.user_status) throw new ConflictException("the user was already disabled");
        ...
        public async Task<int> EnableUserService(int idUser)
        {
            ...
            var user = await _userDAO.DataGetUserById(idUser);
            if (user == null) throw new NotFoundException("User Not Found");
            if (user.user_status) throw new ConflictException("The user's status was already true in the database");
        ...
        public async Task<int> UpgradeRoleFromUserToAdminService(int userId)
        {
            ...
            if (adminIdRole == 0) throw new NotFoundException("role *admin* not found in data base");
            if (adminIdRole == user.user_roleid) throw new ConflictException("The user's role was already admin in the database");
